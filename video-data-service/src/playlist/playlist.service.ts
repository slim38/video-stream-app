...
export class PlaylistService {
  ...
  async update(playlist: PlaylistDto, id: string) {
    const { positionUpdateStatement, positionDeleteStatement } =
      this.getPosDeleteAndUpdateStatement(playlist, id);
    try {
  ...
  private checkForClientError(err: { code: string }, id?: string) {
    ...
    if (err?.code === 'P2002') {
      this.logger.error(
        `Operation failed: specified video is listed several times.`,
      );
      throw new BadRequestException(
        `Spedicified video is listed several times.`,
      );
    }
    if (err?.code === 'P2017') {
      this.logger.error(
        `Operation failed: specified video is not in the playlist.`,
      );
      throw new BadRequestException(
        `Spedicified video is not in the playlist.`,
      );
    }
  }
}
